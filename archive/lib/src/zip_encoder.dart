...
class ZipEncoder {
  ...
  Aes _encryptCompressedData(Uint8List salt) {
    var derivedKey =
        ZipFile.deriveKey(password!, salt, derivedKeyLength: Aes.keySize);
    final keyData = Uint8List.fromList(derivedKey.sublist(0, Aes.keySize));
    final hmacKeyData =
        Uint8List.fromList(derivedKey.sublist(Aes.keySize, Aes.keySize * 2));

    _pwdVer = derivedKey.sublist(Aes.keySize * 2, Aes.keySize * 2 + 2);
    return Aes(keyData, hmacKeyData, Aes.keySize, encrypt: true);
  ...
  void addFile(ArchiveFile file, {bool autoClose = true}) {
    ...
    if (!file.compress) {
      ...
      final bytes =
          Deflate.buffer(file.content as InputStreamBase, level: _data.level)
              .getBytes();
    ...
    if (password != null && compressedData != null) {
      ...
      _output?.aes = _encryptCompressedData(salt);
    }
  ...
  void _writeFile(_ZipFileData fileData, OutputStreamBase output,
    ...
    if (password != null) {
      output.writeBytes(_output!.aes!.mac);
    }
  }
